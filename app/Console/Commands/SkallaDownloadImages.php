...
class SkallaDownloadImages extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'skalla:download-images';
     ...
     * @var string
     */
    protected $description = 'Download Skalla images';
...
//        ---------DOWNLOAD_COLLECTION_IMAGES----------
        ...
        $bar = $this->output->createProgressBar(Skalla::count());
        $bar->start();
...
//       -------------------------------------------------
    ...
    public function download_images($name, $rotate = true, &$err = false): void
    {
        ...
        try {
            if (Storage::disk('skalla')->missing($name_file)) {
                ...
                if ($file != null) {
                    ...
                    Storage::disk('skalla')->put($name_file, $image->encode());
                }
            }
        } catch (Exception $e) {
