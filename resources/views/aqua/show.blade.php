...
@section('meta')
    ...
    <meta property="og:description" content="Коллекция {{$product->brand.' '.$product->collection}}">
...
{{--                    <h1 class="display-6">{{$product->brand}}</h1>--}}
                    ...
                    <h1 class="display-6"><a href="{{route('aquafloor.index')}}"
                       class="link-secondary text-decoration-none">{{$product->brand}}
                    </a></h1>

                    <p class="fs-2">Коллекция:
                        <a href="{{route('aquafloor.collection', $collection)}}"
                           class="link-secondary text-decoration-none">{{$collection}}
                        </a></p>
                ...
                <div class="col-md-6">
                    ...
                    <div id="carouselExample_collection" class="carousel slide carousel-dark pt-3">
                        <div class="carousel-indicators">
                            ...
                            @endphp
                            @foreach($urls as $url_slide_collection)
...
//                        }
                    @endphp
                    <h5 class="mt-4 mb-0">Москва: {{$product->stock->stock_msk??0}} {{$product->stock->unit??0}} ({{$product->stock->stock_msk * $product->meters_in_pack}} {{$product->unit}})</h5>
                    {{--                    @if ($stock_spb)--}}
                    ...
                    <p class="mt-4">Актуально на <span
                            class="{{$text_color}} fw-bolder">{{$product->stock->updated_at->format('d.m.Y')}}</span></p>
            ...
            <div class="row">
                <div class="col-md-6">
                    <table class="table table-striped">
                        ...
                        @endif
                        @if($product->collection_relation->category)
                            <tr>
                                <th scope="row">Категория</th>
                                <td>{{$product->collection_relation->category}}</td>
                            </tr>
                        @endif
                        ...
                        @if($product->decor)
                            <tr>
                                <th scope="row">Декор</th>
                                <td>{{Str::replace(';', ' ', $product->decor)}}</td>
                            </tr>
                        @endif
                        @if($product->color)
                            <tr>
                        ...
                        @if($product->meters_in_pack)
                            <tr>
                                ...
                                <td>{{$product->meters_in_pack}} м2</td>
                            </tr>
                        @endif
                        @if($product->count_in_pack)
                            <tr>
                                <th scope="row">Количество в упаковке</th>
                                <td>{{$product->count_in_pack}} шт</td>
                            </tr>
                        @endif
                        @if($product->podlozhka)
                            <tr>
                                <th scope="row">Подложка</th>
                                <td>{{$product->podlozhka}}</td>
                            </tr>
                        @endif
                        @if($product->faska)
                            <tr>
                        ...
                        @endif
                        @if($product->type_soedinen)
                            <tr>
                                <th scope="row">Тип соединения</th>
                                <td>{{$product->type_soedinen}}</td>
                            </tr>
                        @endif
                        @if($product->osnova)
                            <tr>
                        ...
                        @if($product->textura)
                            <tr>
                                <th scope="row">Текстура</th>
                                <td>{{$product->textura}}</td>
                            </tr>
                        @endif
                        @if($product->zashit_sloy)
                            <tr>
                                <th scope="row">Защитный слой</th>
                                <td>{{$product->zashit_sloy}} мм</td>
                            </tr>
                        @endif
                        @if($product->class)
                            <tr>
                                <th scope="row">Класс нагрузки</th>
                                <td>{{$product->class}}</td>
                            </tr>
                        @endif
                        @if($product->vlagostoikost)
                            <tr>
                                <th scope="row">Влагостойкость</th>
                                <td>{{$product->vlagostoikost}}</td>
                            </tr>
                        @endif
                        </tbody>
                    </table>
