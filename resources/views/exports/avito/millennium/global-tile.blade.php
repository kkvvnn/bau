...
//                }
            ...
            $description .= '<li><strong>В упаковке: </strong>' . $product->count_in_pack . ' шт</li>';
            }
            if($product->meters_in_pack != null && $product->meters_in_pack != $product->count_in_pack) {
            $description .= '<li><strong>В упаковке: </strong>' . $product->meters_in_pack . ' кв.м.</li>';
            }
        ...
        $description .= '<p><strong><em>Керамогранит Global Tile доставляется со склада в г.Самара под заказ. Минимальная сумма заказа 20000 р.</em></strong></p>';
...
//            if (mb_strlen($title) < 42) { $title = $product->Producer_Brand . ' ' . $title; }
    ...
    @php
        $code = $product->vendor_code.'_millennium';
        $video = '';
    @endphp
