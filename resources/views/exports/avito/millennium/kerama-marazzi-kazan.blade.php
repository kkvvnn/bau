{{-----KERAMA-MARAZZI-KAZAN-----}}

@foreach($kerama_marazzi as $product)

    @php
        $product_name = $product->Name;
        if ($product->Category == 'Керамогранит') {
            $product_name .= ' Керамогранит';
        }
        if (stripos($product->Name, 'обрезной') !== false) {
            $product_name .= ' Керамогранит';
        }

        $product_type = avito_type($product_name);
...
//        $description .= '<ul>';
//        $description .= '<li>Склад СПб: ' . round($product->spb->balanceCount, 2) . ' ' . $product->MainUnit . '</li>';
//        $description .= '</ul>';
//        $description .= '<p><em>(актуальную информацию о наличии уточняйте у менеджера)</em></p>';
        ...
        if ($product->Package_Value != null && $product->Package_Value != $product->PCS_in_Package) {
           $description .= '<li><strong>В упаковке: </strong>' . $product->Package_Value . ' кв.м</li>';
        }
    ...
    @php
        ...
        if (mb_strlen($title) > 50) {
            ...
            $title = str_replace(' обрезной', '', $title);
        ...
        if (mb_strlen($title) > 50) {
            $title = str_replace(' лаппатированный', '', $title);
        }
        ...
        if (mb_strlen($title) > 50) {
            $title = str_replace(' матовый', '', $title);
        }

        if (mb_strlen($title) > 50) {
            $title = str_replace(' светлый', '', $title);
        }

        if (mb_strlen($title) > 50) {
            $title = str_replace(' глянцевый', '', $title);
        }

        if (mb_strlen($title) > 50) {
            $title = str_replace(' Керамогранитсерый', '', $title);
        }

        if (mb_strlen($title) > 50) {
            $title = str_replace(' декорированный', '', $title);
        }
    ...
    @php
        ...
        $code = $product->Element_Code.'_kazan_millennium';
        $video = '';
