...
@foreach($primavera as $product)
    ...
    @php
        ...
        $description .= '<p>Керамическая плитка и керамогранит '.$product->brand.'.</p>';
...
//                }
            ...
            $description .= '<li><strong>В упаковке: </strong>' . str_replace(',', '.', $product->square_in_pack) . ' кв.м.</li>';
            }
...
//            $title = preg_replace('/\d+-\d+-\d+-\d+/', '', $title);
        ...
        $code = $product->vendor_code.'_primavera_millennium';
        $video = '';
